...
#include "detail/cpu_opcode_cycles.h"
...
MCycles Execute(CpuState &state, Reg8 opcode)
{
    ...
    state.cbOpCodeCycles = {0};

    if (opcode == 0x10) // STOP
    {
        state.pauseClockFunc();
    }

    s_Instructions[opcode](state);
    if (state.cbOpCodeCycles.cycles != 0)
    {
        return state.cbOpCodeCycles;
    }
